...
get_price_method1() {
    local response=$(curl -s -m 10 "http://qt.gtimg.cn/q=usSPCX" | iconv -f gbk -t utf-8 2>/dev/null)
...
try:
    ...
    if '~' in data:
        # 腾讯财经数据格式: v_usSPCX=\"字段0~字段1~...\"
        parts = data.split('~')
        if len(parts) > 35:
            # 字段说明:
            # 3: 最新价, 4: 昨收价, 5: 今开, 6: 成交量, 33: 最高, 34: 最低
...
# 方法2: 使用新浪财经（备用）
get_price_method2() {
    local response=$(curl -s -m 10 "https://hq.sinajs.cn/list=gb_spcx" | iconv -f gbk -t utf-8 2>/dev/null)
    if [ $? -eq 0 ] && [ -n "$response" ] && [[ "$response" == *","* ]]; then
        echo "$response" | python3 -c "
import sys
try:
...
# 检查是否成功获取价格
if [ -z "$PRICE_DATA" ]; then
    echo "${TIMESTAMP} - ERROR: 无法获取 SPCX.O 股价（所有数据源均失败）" | tee -a "${LOG_FILE}"
    echo "${TIMESTAMP} - INFO: 请检查网络连接或数据源是否可用" | tee -a "${LOG_FILE}"
    echo "${TIMESTAMP} - INFO: 东方财富网显示 SpaceX (SPCX) 最新价格约为 \$145.30 (2026-07-13参考价)" | tee -a "${LOG_FILE}"
    exit 1
fi
...
# 记录价格
echo "${TIMESTAMP} - SPCX.O 股价: \$${PRICE} ${CHANGE_STR} | 开:\$${OPEN} 高:\$${HIGH} 低:\$${LOW} 量:${VOLUME} [来源: ${METHOD}]" | tee -a "${LOG_FILE}"
