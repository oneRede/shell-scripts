...
LOG_FILE="${LOG_DIR}/amzn_price_$(date +%Y%m%d).log"
...
get_price_method1() {
    local response=$(curl -s -m 10 "http://qt.gtimg.cn/q=usAMZN" | iconv -f gbk -t utf-8 2>/dev/null)
...
if [ -z "$PRICE_DATA" ]; then
    echo "${TIMESTAMP} - ERROR: 无法获取 AMZN 股价（所有数据源均失败）" | tee -a "${LOG_FILE}"
    exit 1
fi
...
echo "${TIMESTAMP} - AMZN 股价: \$${PRICE} ${CHANGE_STR} | 开:\$${OPEN} 高:\$${HIGH} 低:\$${LOW} 量:${VOLUME}${PE_INFO} [来源: ${METHOD}]" | tee -a "${LOG_FILE}"

CSV_FILE="${DATA_DIR}/amzn_price.csv"
if [ ! -f "${CSV_FILE}" ]; then
