...
LOG_FILE="${LOG_DIR}/aapl_price_$(date +%Y%m%d).log"
...
get_price_method1() {
    local response=$(curl -s -m 10 "http://qt.gtimg.cn/q=usAAPL" | iconv -f gbk -t utf-8 2>/dev/null)
...
try:
    ...
    if '~' in data:
        ...
        if len(parts) > 35:
            # 字段说明:
            # 3: 最新价, 4: 昨收价, 5: 今开, 6: 成交量
            # 33: 最高, 34: 最低, 39: 动态P/E, 51: 静态P/E
...
# 检查是否成功获取价格
if [ -z "$PRICE_DATA" ]; then
    echo "${TIMESTAMP} - ERROR: 无法获取 AAPL 股价（所有数据源均失败）" | tee -a "${LOG_FILE}"
...
CSV_FILE="${DATA_DIR}/aapl_price.csv"
if [ ! -f "${CSV_FILE}" ]; then
